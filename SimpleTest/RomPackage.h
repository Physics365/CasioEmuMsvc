...
#include <filesystem>
#include <fstream>
void WriteFile(std::filesystem::path pth, const std::vector<unsigned char>& f) {
...
void ReadFile(std::filesystem::path pth, std::vector<unsigned char>& f) {
...
void WriteFile(std::filesystem::path pth, const auto& f) {
	std::ofstream fs(pth, std::ios::binary | std::ios::out);
	if (!fs)
		throw std::runtime_error("Cannot open file.");
	Binary::Write(fs, f);
...
void ReadFile(std::filesystem::path pth, auto& f) {
	std::ifstream fs(pth, std::ios::binary | std::ios::in);
	if (!fs)
		throw std::runtime_error("Cannot open file.");
	Binary::Read(fs, f);
